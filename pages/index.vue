<template>
  <div>
    <Header/>
      <section>
        <img :src="home.main_photo" alt="">
        <div>
          <h2>Recomendados</h2>
          <ul class="lista-destinos">
            <li v-for="destino in home.recommended" :key="destino.link_url">
              <a :href="destino.link_url">
                <img :src="destino.item_photo" :alt="destino.location">
              </a>
              <div>
                <p>{{ destino.location }}</p>
                <p>{{ destino.stars}}</p>
                <ul>
                  <li v-for="(star, index) in Number(destino.stars)" :key="index">
                    {{ index }}
